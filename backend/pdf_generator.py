...
def _fmt_money(amount, currency: str = "AED"):
    try:
...
def _build_document(doc_type: str, doc_data: dict, client_doc: dict, user_doc: dict) -> bytes:
    ...
    for it in (doc_data.get('items') or []):
        ...
        table_data.append([
            ...
            Paragraph(_fmt_money(rate, currency), ParagraphStyle('r', parent=body, alignment=TA_RIGHT)),
            Paragraph(_fmt_money(amt, currency), ParagraphStyle('r', parent=body, alignment=TA_RIGHT)),
        ])
